...
<div class = "blue">
**Background**
Need to run the "setting_all_names.Rmd" code file first before using this code file.

* Summary:
  + There are 36 unique to salmon metabolites, in which 25 Metabolites are named and 11 are unnamed Metabolites.
...
datatable(as.data.frame(food),rownames=F,extensions=c("Buttons","ColReorder"),
          ...
          class="cell-border stripe", caption="Original Salmon Dataset")

write.csv(food, "./food_table/Original_Dataset.csv", row.names=F)
```

Assigning Metabolite Names Such as: "M.1", "M.2", ... to Unique to Salmon Metabolites
```{r}
metabolite_info <- as.data.frame(read.table("./met_info.txt",header=T))

Metabolite <- paste("M.",1:nrow(metabolite_info), sep = "")
...
write.csv(metabolite_info, "./food_table/Metabolite_Information.csv", row.names=F)
...
write.csv(missing_summary, "./food_table/Summary_Missing_less_75_Percent.csv", row.names=F)
```
...
write.csv(cross_section_ranked, "./food_table/Ranking_metabolites.csv", row.names=F)
```
...
datatable(as.data.frame(difference_ranked_change_analysis),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colreorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50)),
          class="cell-border stripe", caption="Change in Ranks For Unique to Salmon Metabolites within Each Trial")

write.csv(difference_ranked_change_analysis,
          "./food_table/Change_in_Ranks_Metabolites.csv", row.names=F)
```
...
write.csv(change_analysis_change_no_cov,
          "./food_table/Change_in_Ranks_Primary_Analysis.csv", row.names=F)
...
datatable(as.data.frame(change_analysis_change_cov),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50)),
          class="cell-border stripe", caption="LMM for Change in Ranks of Unique to Salmon Metabolites with Covariates")
write.csv(change_analysis_change_cov,
          "./food_table/Change_in_Ranks_Secondary_Analysis.csv", row.names=F)
```
...
datatable(as.data.frame(method_M_no_cov),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50,100)),
          class="cell-border stripe", caption="Stratified LMM for Change in Ranks of Unique to Salmon Metabolites w/o Covariates for Males")
write.csv(method_M_no_cov, "./food_table/Stratified_Change_in_Ranks_Primary_Males.csv", row.names=F)
```
...
datatable(as.data.frame(method_M_cov),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50,100)),
          class="cell-border stripe", caption="Stratified LMM for Change in Ranks of Unique to Salmon Metabolites with Covariates for Males")
write.csv(method_M_cov, "./food_table/Stratified_Change_in_Ranks_Secondary_Males.csv", row.names=F)
```
...
datatable(as.data.frame(method_F_no_cov),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50,100)),
          class="cell-border stripe", caption="Stratified LMM for Change in Ranks of Unique to Salmon Metabolites w/o Covariates for Females")
write.csv(method_F_no_cov, "./food_table/Stratified_Change_in_Ranks_Primary_Females.csv", row.names=F)
```
...
datatable(as.data.frame(method_F_cov),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50,100)),
          class="cell-border stripe", caption="Stratified LMM for Change in Ranks of Unique to Salmon Metabolites with Covariates for Females")
write.csv(method_F_cov, "./food_table/Stratified_Change_in_Ranks_Secondary_Females.csv", row.names=F)
```
...
There are Two Approaches to Assess this Association. First Method is to Rank the Normalized Values within Each Metabolites, and then Apply Linear Mixed Model(LMM) to Assess Association between Unique to Salmon Metabolites with Health Outcomes.Second Method is to use the Ranked Metabolites From First Method and then Sum Metabolites Rankings for Each Individual and Specific Period and then Apply LMM to Assess Association between Unique to Salmon Metabolites with Health Outcomes. 
...
write.csv(cross_section_out_period,
          "./food_table/Cross_section_analysis_Health_Metabolites.csv", row.names=F)
```
...
There were 69 Possible Associations Between Health Outcomes and Metabolites That had a FDR value Below 0.05. In Which 11 were Related to ApoB, 16 to Chol_T, 2 to Chol_T_HDL, 1 to CRP, 14 to HDL, 3 to HDL_LDL, 13 to LDL, and 9 to TG.  
```{r}
...
write.csv(cross_section_out_pre,
          "./food_table/Stratified_Cross_section_health_Metabolites_Pre_int.csv", row.names=F)
```
...
write.csv(cross_section_out_post,
          "./food_table/Stratified_Cross_section_health_Metabolites_Post_int.csv", row.names=F)
```
...
write.csv(comp_time,
          "./food_table/Compare_stratified_Pre_vs_Post_int.csv", row.names=F)
```
...
write.csv(out_per_1,
          "./food_table/Stratified_health_Metabolites_Period_1_int.csv", row.names=F)
```
...
write.csv(out_per_2,
          "./food_table/Stratified_health_Metabolites_Period_2_int.csv", row.names=F)
```
...
write.csv(out_per_3,
          "./food_table/Stratified_health_Metabolites_Period_3_int.csv", row.names=F)
```
...
write.csv(out_per_4,
          "./food_table/Stratified_health_Metabolites_Period_4_int.csv", row.names=F)
```
...
When Looking at the Sum of Ranked Unique to Salmon Metabolites, None of the Health Outcomes had FDR Value Below 0.05. 
```{r message=FALSE}
...
datatable(as.data.frame(cross_section_out_sum),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50)),
          class="cell-border stripe", caption="LMM Results For Sum of Ranks for Unique to Salmon Metabolites with Health Outcomes Cross-Sectionally")
write.csv(cross_section_out_sum,
          "./food_table/Cross_section_sum_Health_Metabolites.csv", row.names=F)
```
...
In this section we use used the previously ranked normalized values of "cross_section_ranked", Next Calculate Difference in Rank within Each Trial. First Method uses the Difference in Ranks to Apply LMM to Assess Association with Health Outcomes. Second Method uses The Difference in Ranks to Sum Metabolites Ranking for Each Individual and Specific Trial, and then Apply LMM to Assess Association between Change in Ranked Unique to Salmon Metabolites and Change in Health Outcomes. Also Each of These Approaches has two Sub-Methods to Assess Change in Ranks of Unique to Salmon Metabolites. First Formula to Assess change in Ranks of Unique to Salmon Metabolite Excludes any Covariates. Second Formula to Assess Change in Ranks of Unique to Salmon Metabolite Includes Covariates Such as "Age" and "Sex", in which "Age" is scaled.
...
datatable(as.data.frame(change_analysis_out),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50)),
          class="cell-border stripe", caption="LMM Results on For Change in Ranks of Unique to Salmon Metabolites wtih Change in Health Outcomes")
write.csv(change_analysis_out,
          "./food_table/change_in_Health_Metabolites.csv", row.names=F)
```
...
datatable(as.data.frame(sig_2_out),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50)),
          class="cell-border stripe", caption="Significant Metabolites of For Change in Ranks of Unique to Salmon Metabolites wtih Change in Health Outcomes")
```
...
#### Association Between Sum of Change in Salmon Metabolites Ranks and Change in Health Outcomes
When Looking at the Change in Sum of Ranked Unique to Salmon Metabolites, None of Health Outcomes had a FDR Below 0.05 Value.
```{r message=FALSE}
...
datatable(as.data.frame(change_analysis_sum_out),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50)),
          class="cell-border stripe", caption="LMM Results for Sum of Change in Ranks of Unique to Salmon Metabolites with Change in Health Outcomes")
write.csv(change_analysis_sum_out,
          "./food_table/Sum_of_change_in_Ranks_Metabolites_Health.csv", row.names=F)
```
...
Sum of Ranks of Unique to Salmon Metabolites have FDR Value Below 0.05
```{r message=FALSE}
...
datatable(as.data.frame(change_analysis_sum_change_no_cov),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50)),
          class="cell-border stripe", caption="LMM for Change in Sum of Ranks in Unique to Salmon Metabolites Without Covariates")
write.csv(change_analysis_sum_change_no_cov,
          "./food_table/Sum_of_change_in_Ranks_Metabolites_w_o_cov.csv", row.names=F)
```
...
datatable(as.data.frame(change_analysis_sum_change_cov),rownames=F,extensions=c("Buttons","ColReorder"),
          options=list(dom="Bfrtipl",scrollY=TRUE,scrollX=TRUE,colReorder=TRUE,buttons=c("excel","csv"),
                       pageLength=10,lengthMenu=c(10,25,50)),
          class="cell-border stripe", caption="LMM for Change in Sum of Ranks in Unique to Salmon Metabolite with Covariates")
write.csv(change_analysis_sum_change_cov,
          "./food_table/Sum_of_change_in_Ranks_Metabolites_with_cov.csv", row.names=F)
```
