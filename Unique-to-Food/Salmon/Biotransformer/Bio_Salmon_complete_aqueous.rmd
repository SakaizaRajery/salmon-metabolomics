---
title: "Aqueous Biotransformer Salmon Analysis"
output:
  html_document:
    df_print: paged
    toc: yes
...
<div class = "blue">
**Background**
Need to run the "aqueous_setting_names.Rmd" code file first before using this code file.

* Summary:
  + There are 31 predicted salmon metabolites.
  + There are 2 metabolites that had more than 75% total missing, and thus they were excluded from further analysis. 
...
food <- as.data.frame(read.table(paste("./merged_file_",file.name,".txt",sep = ""),check.names=F, header = T))
...
Assigning Metabolite Names Such as: "M.1", "M.2", ... to predicted aqueous Salmon compounds
```{r}
...
#na_df <- food[,Metabolite][rowSums(is.na(food[,Metabolite])) > 0,]
...
Removed 2 Metabolites 31, and 29 Which had a Total Missing more than 75%
```{r}
...
This Plot Compares Presence for Each Metabolite Based on Pre vs Post. This Plot Shows That predicted Salmon Metabolites had a Higher Presence in "Post", Indicating Salmon Metabolites were Often Seen After Consumption of Salmon
```{r}
ggplot()+
...
QC_val <- QC_val[!is.na(QC_val$Type),]
#QC_val <- QC_val[,colSums(is.na(QC_val))<nrow(QC_val)]
QC_Metabolite <- colnames(QC_val[,11:ncol(QC_val)])

QC_mean <- sapply(QC_val[QC_Metabolite],mean,na.rm=T)
QC_SD <- sapply(QC_val[QC_Metabolite],sd,na.rm=T)
QC_CV <- QC_SD/QC_mean

CV_summary <- as.data.frame(cbind(QC_CV,QC_mean,QC_SD))

#Since all metabolite have QC value below 0.3 except for M.27 whose QC is NA, keep all metabolite
#Metabolite <- rownames(CV_summary[CV_summary$QC_CV < 0.3,])
Metabolite<-rownames(CV_summary)
...
method_M_cov <- merge(method_M_cov,metabolite_info,by="Metabolite")
...
write.csv(method_M_cov, paste("./food_table/Stratified_Change_in_Ranks_Secondary_Males_",file.name,".csv",sep = ""), row.names=F)
```
...
method_F_no_cov <- merge(method_F_no_cov,metabolite_info,by="Metabolite")
...
write.csv(method_F_no_cov, paste("./food_table/Stratified_Change_in_Ranks_Primary_Females_",file.name,".csv",sep = ""), row.names=F)
```
...
method_F_cov <- merge(method_F_cov,metabolite_info,by="Metabolite")
...
cross_section_out_pre <- cross_section_out_pre[,-2]  
names(cross_section_out_pre)[names(cross_section_out_pre) == 'Out_type_Age'] <- "type"
...
## Changing Class to Numeric
cross_section_out_pre[,5:ncol(cross_section_out_pre)] <- lapply(cross_section_out_pre[,5:ncol(cross_section_out_pre)], function(x) {as.numeric(x)})
...
cross_section_out_post <- cross_section_out_post[,-2]  
names(cross_section_out_post)[names(cross_section_out_post) == 'Out_type_Age'] <- "type"
...
## Changing Class to Numeric
cross_section_out_post[,5:ncol(cross_section_out_post)] <- lapply(cross_section_out_post[,5:ncol(cross_section_out_post)], function(x) {as.numeric(x)})
...
write.csv(cross_section_out_post, paste("./food_table/Stratified_Cross_section_health_Metabolites_Post_int_",file.name,".csv",sep = ""), row.names=F)
```
...
out_per_1 <- out_per_1[,-2]  
names(out_per_1)[names(out_per_1) == 'Out_type_Age'] <- "type"
...
## Changing Class to Numeric
out_per_1[,5:ncol(out_per_1)] <- lapply(out_per_1[,5:ncol(out_per_1)], function(x) {as.numeric(x)})
...
write.csv(out_per_1, paste("./food_table/Stratified_health_Metabolites_Period_1_int_",file.name,".csv",sep = ""), row.names=F)
```
...
When Looking at the Sum of Ranked prediced Salmon Metabolites, None of the Health Outcomes had FDR Value Below 0.05. 
```{r message=FALSE}
...
change_analysis_out <- change_analysis_out[,-2]  
names(change_analysis_out)[names(change_analysis_out) == 'Out_type_Age'] <- "type"
...
## Changing Class to Numeric
change_analysis_out[,5:ncol(change_analysis_out)] <- lapply(change_analysis_out[,5:ncol(change_analysis_out)], function(x) {as.numeric(x)})
...
```{r}
outcomes <- c("SBP","DBP","Insulin","Chol_T","TG","LDL","HDL","CRP","ApoB","HDL_LDL","Chol_T_HDL")
